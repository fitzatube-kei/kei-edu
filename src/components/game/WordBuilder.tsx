...
export default function WordBuilder({ words, onComplete, onExit }: WordBuilderProps) {
  ...
  const [showWordPreview, setShowWordPreview] = useState(true);
  ...
  // Per-word preview: show the word before puzzle
  if (showWordPreview) {
    return (
      <div className="w-full h-full flex flex-col">
        ...
        {/* Progress bar */}
        <div className="w-full h-[8px] bg-[#1B0440] rounded-full overflow-hidden mb-6">
          <motion.div
            initial={{ width: 0 }}
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          ...
          <motion.span
            ...
            className="inline-block text-[11px] text-white font-bold uppercase tracking-wide bg-[#8A4AEF] px-4 py-1.5 rounded-[8px] mb-4"
          >
          ...
          >
            <span className="text-[72px] xs:text-[80px] sm:text-[96px] font-black text-white leading-none">
              {currentWord.korean}
            </span>
          </motion.div>
          ...
          {currentWord.romanization && (
            <motion.p
              ...
              className="text-white/50 text-[16px] mb-2"
            >
              {currentWord.romanization}
            </motion.p>
          )}
          ...
          <motion.p
            ...
            className="text-[#B4D700] text-[20px] sm:text-[24px] font-bold mb-6"
          >
            {getWordTranslation(currentWord.korean, language as SupportedLanguage) || currentWord.english}
          </motion.p>
          ...
          >
            ...
            <span className="text-[14px] font-semibold">Tap to listen</span>
          </motion.button>
        </div>
        ...
        <motion.button
          ...
          className="w-full flex items-center justify-center gap-2 bg-[#B4D700] text-[#1B0440] rounded-[14px] font-bold py-4 text-[16px] mb-2"
        >
